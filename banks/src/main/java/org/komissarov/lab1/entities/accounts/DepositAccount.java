...
/**
 * This class represents a deposit account in a bank.
 * It implements the IAccount interface and provides the necessary methods for managing a deposit account.
 */
public class DepositAccount implements IAccount {
    ...
    /**
     * Constructs a new DepositAccount with the specified client ID and number of days.
     * The balance of the new account is initialized to 0.
     *
     * @param clientId the ID of the client who owns this account
     * @param days     the number of days for the deposit
     */
    public DepositAccount(UUID clientId, int days) {
        this.balance = 0;
     ...
     * The balance is increased by the product of the balance, the bank's deposit percentage,
    ...
    public void countBalance(int days, Bank bank) {
        ...
        SortedMap<Double, Double> depositPercents = bank.getDepositPercentage();
        double percent = depositPercents.get(depositPercents.headMap(balance).lastKey());

        balance += balance * percent * days / 365;
        this.days -= this.days <= 0 ? 0 : days;
    }
}
